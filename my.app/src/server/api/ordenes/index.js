  ...
  try {
    ...
    if (req.method === "POST") {
      const { data: carritoItems, error: carritoError } = await supabase
        .from("carrito")
        .select(
          "producto_id, producto_variante_id, cantidad, productos(id, nombre, precio, stock), producto_variantes(id, precio, stock, estado)"
        )
      ...
      if (!carritoItems.length) {
        return errorResponse(res, 400, "El carrito está vacío", "EMPTY_CART");
      }

      for (const item of carritoItems) {
        const stockDisponible = item.producto_variantes
          ? Number(item.producto_variantes.stock)
          : Number(item.productos?.stock);

        if (!item.productos || stockDisponible < item.cantidad) {
          return errorResponse(res, 400, "Stock insuficiente", "INSUFFICIENT_STOCK");
        }
      }

      const total = carritoItems.reduce((sum, item) => {
        const precio = item.producto_variantes
          ? Number(item.producto_variantes.precio)
          : Number(item.productos.precio);
    ...
    return errorResponse(res, 500, "Error al procesar órdenes", "SERVER_ERROR");
  }
}
